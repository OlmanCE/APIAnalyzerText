...
def sample_classify_text(input_data):
    ...
    type_ = language_v1beta2.Document.Type.PLAIN_TEXT
    language = "es"
    document = {"content": text_content, "type": type_, "language": language}
    # Specify model version for content categorization
    content_categories_version = language_v1beta2.ClassificationModelOptions.V2Model.ContentCategoriesVersion.V2
...
def txt_to_json_messages_only(filepath, lang='auto'):
    ...
    pattern = {
        'en': r'(\d{2}/\d{2}/\d{2}), (\d{1,2}:\d{2}\s[ap]m) - (.*?): (.*)',
        'es': r'(\d{2}/\d{2}/\d{2}), (\d{1,2}:\d{2}\s[a|p]\. m\.) - (.*?): (.*)'
    ...
    with open(filepath, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if match:
                ...
                if "end-to-end encrypted" not in message and "cifrado de extremo a extremo" not in message and "(file attached)" not in message:
                    print("Mostar mensaje: ", message)
    ...
    print("Conversación: ", conversation)
    # Devolver el contenido en el formato esperado para sample_classify_text
